...
async function exportIcons(input) {
  ...
  for (const [imageIndex, image] of images.entries()) {
    ...
    const scenePath = generatedPathFromUrl(image.scene_url);
    if (scenePath && fs.existsSync(scenePath)) {
      const sceneFilename = `${productName}_icon${suffix}_v${version}_scene.svg`;
      fs.copyFileSync(scenePath, path.join(packageDir, sceneFilename));
      files.push({
        image_id: image.image_id,
        size: "scene",
        filename: sceneFilename,
        url: `/exports/${packageId}/${sceneFilename}`,
      });
    }
  }
  ...
  return {
    ...
    manifest_url: `/exports/${packageId}/manifest.json`,
    files,
  };
}
