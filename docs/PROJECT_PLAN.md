...
- 保存 S6 用户选择和 S8 导出记录。
- 前端增加历史任务列表。
- 增加竞品实体 MVP 表，保存基础商店信息、Icon、评分结果和上次生成时间。
- 增加竞品评分配置表，支持单项分值、自动入库阈值、自动排除阈值人工配置。
- 增加竞品看板，展示总分、分层结果、配置版本、上次生成时间和待更新状态。
- 支持用户手动更新竞品评分。
- 支持定时自动更新待刷新竞品评分，每次更新使用当前最新配置。
...
- 可以查看每次生成使用的 Prompt。
- 可以重新导出历史选中 Icon。
- 竞品看板可以按自动入库、人工审核、自动排除、待更新筛选。
- 配置调整后，已存竞品会标记为待更新，不直接覆盖历史评分。
- 手动更新和定时更新都会记录使用的配置版本与上次生成时间，时间精确到分钟。
