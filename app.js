...
async function apiPost(path, payload) {
  ...
  try {
    ...
    throw new Error(apiBase ? "Cannot connect to local service. Open http://localhost:8787 or restart node server.js." : "Cannot connect to online service. Please retry later or check Railway deployment status.");
  }
...
function stageTwo() {
  return `
    ...
    <ul class="mini-list">
      <li>Naming: {sanitizeName(state.product || "product")}_icon_v1_{size}px.png</li>
      <li>Delivery: PNG files in 1024/512/256/128/64 and one ZIP package.</li>
      <li>The ZIP excludes scene SVG files and manifest JSON.</li>
    </ul>
  `;
}
...
function stageEight() {
  return `
    <h3>多尺寸导出与交付</h3>
    <p class="stage-copy">Only resized PNG icon files will be exported in the ZIP package.</p>
...
function getStageOutput() {
  ...
  const stageOutputs = {
    ...
    8: {
      ...
      excludes: ["scene SVG", "manifest JSON"],
    },
  };
